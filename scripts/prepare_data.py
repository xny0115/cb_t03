...
def sentence_iterator(data_dir: Path):
    ...
    logging.info(f"Found {len(text_files)} files. Reading them...")
    if not text_files:
    ...
    for file_path in text_files:
        try:
            ...
            logging.warning(f"UTF-8 decoding failed for {file_path}. Trying 'cp949'.")
            try:
                with codecs.open(file_path, 'r', encoding='cp949', errors='strict') as f:
                    for line in f:
                        yield line.strip()
            except Exception as e:
                logging.error(f"Could not read {file_path}. Skipping. Error: {e}")

def train_tokenizer(data_dir: Path, model_path_prefix: str, vocab_size: int, model_type: str = 'bpe'):
