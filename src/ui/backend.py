...
class WebBackend:
    ...
    def _try_service_call(self, func, *args, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # Ensure the result is a dictionary, as the frontend expects it.
            if isinstance(result, dict):
                return result
            # For functions that don't return a dict (like delete_model), wrap the result.
    ...
    def delete_model(self) -> Dict[str, Any]:
        # This one has a simple boolean return, wrap it manually for clarity.
        try:
            ok = self._svc.delete_model()
            return {"success": ok, "msg": "deleted" if ok else "no_model", "data": None}
        except Exception as e:
            logger.exception("An error occurred during model deletion")
            return {"success": False, "msg": str(e), "data": None}
    ...
    def get_status(self) -> Dict[str, Any]:
        # get_status is expected to be lightweight and non-failing, but wrapping is safer.
        return self._try_service_call(self._svc.get_status)
