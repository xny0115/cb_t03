...
class ChatbotService:
    ...
    def __init__(self) -> None:
        ...
        self.model: Optional[Seq2SeqTransformer | DummyModel | HFModel] = None
        self._config = load_config()

        tokenizer_path = self._config.get("tokenizer_path", "models/spm_bpe_8k.model")
        ...
        if hf_name:
            ...
            if model_files:
                # Exclude checkpoint files from being loaded directly
                model_files = [p for p in model_files if 'checkpoint' not in p.name]
                if model_files:
    ...
    def start_training(self, mode: str) -> Dict[str, Any]:
        ...
        if mode == "pretrain":
            ...
            # Pass the existing model to the training function to resume training
            trained_model = pretrain(dataset, self._config, model=self.model)
        else:
            if mode == "additional_finetune":
    ...
    def delete_model(self) -> bool:
        ...
        all_model_files = list(self.model_dir.glob("*.pth"))
        for fp in all_model_files:
            try:
                fp.unlink()
                deleted = True
