# 변경 이력
## v1.52
- 이어학습 시 체크포인트 에폭과 설정 에폭이 따로 계산되어 학습 루프가 즉시 종료되던 버그를 수정했습니다.
- `start_epoch + num_epochs`까지 반복하도록 변경해 체크포인트 이후에도 지정한 횟수만큼 학습이 진행됩니다.
- 진행률 로그에 총 에폭 수를 사용해 정확한 정보를 제공합니다.
## v1.51
