...
### 주요 변경 사항
- **학습 플로우 재정의**: 사용자가 '처음부터 학습'과 '이어서 학습'을 명확히 구분하여 실행할 수 있도록 학습 시작 로직을 전면 재정의했습니다.

### 세부 변경 내역
- **문제점**: 기존에는 모델 파일(`.pth`)만 삭제하고 학습을 시작할 경우, 체크포인트 파일(`training_state.pth`)이 남아있어 사용자의 의도와 다르게 '이어서 학습'이 강제되는 문제가 있었습니다. 이로 인해 학습이 정상적으로 진행되지 않는 현상이 발생했습니다.
- **개선 사항**:
    1.  **UI 버튼 명칭 변경**: 사용자의 혼동을 줄이기 위해 '추가 파인튜닝' 버튼의 명칭을 '이어학습'으로 변경하고, 내부 호출 모드를 `resume_training`으로 수정했습니다. (`ui.html`)
    2.  **학습 모드별 동작 분리**:
        -   '사전학습' 또는 '파인튜닝' 버튼 클릭 시: 항상 새로운 학습이 시작되도록, 기존 체크포인트 파일이 존재하면 자동으로 삭제하는 로직을 추가했습니다.
        -   '이어학습' 버튼 클릭 시: 기존과 동일하게 체크포인트 파일이 존재할 경우에만 학습을 이어가도록 수정했습니다.
    3.  **데이터 로딩 로직 명확화**: '이어학습' 시에는 '추가 파인튜닝'용 데이터를 사용하도록 데이터 로딩 경로를 명확히 지정했습니다. (`src/service/service.py`)

이 변경을 통해 사용자는 버튼을 통해 직관적으로 학습 방식을 선택할 수 있으며, '학습이 진행되지 않던' 근본적인 문제를 해결했습니다.
